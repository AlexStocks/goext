...
func (t *TestSuite) TestFlock_TryLock(c *C) {
	...
	locked, err = NewFlock(t.path).TryLock()
	c.Assert(err, IsNil)
	c.Check(locked, Equals, false)
}
...
func (t *TestSuite) TestFlock_TryRLock(c *C) {
	...
	flock2.Unlock()
	t.flock.Lock()
	locked, err = NewFlock(t.path).TryRLock()
	c.Assert(err, IsNil)
	c.Check(locked, Equals, false)
}
...
func (t *TestSuite) TestFlock_TryLockContext(c *C) {
	...
	cancel()
	locked, err = NewFlock(t.path).TryLockContext(ctx, time.Second)
...
func (t *TestSuite) TestFlock_TryRLockContext(c *C) {
	...
	// context already canceled
	cancel()
	locked, err = NewFlock(t.path).TryRLockContext(ctx, time.Second)
	c.Assert(err, Equals, context.Canceled)
	c.Check(locked, Equals, false)
