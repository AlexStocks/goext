...
// ref: https://github.com/dgryski/go-jump/blob/master/jump.go
func TestHash(t *testing.T) {
	...
	for _, tt := range tests {
		for i, v := range tt.bucket {
			if got := JumpConsistentHash(tt.key, i+1); got != v {
...
func TestGolden(t *testing.T) {
	...
	for i, v := range golden100 {
		if g := JumpConsistentHash(uint64(i), 100); g != v {
	...
	for _, tt := range tests {
		if g := JumpConsistentHash(tt.k, tt.buckets); g != tt.out {
